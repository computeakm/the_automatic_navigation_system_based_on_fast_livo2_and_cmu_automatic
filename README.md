...
不使用gazebo的分支，理论上可以直接部署在机器上，使用ros2版本为humbel
部署流程 （ht_ws文件夹）
1. LIVOX-SDK(不要放在包下面哦ovo)
    ...
    cmake .. && make -j
    sudo make install
2. livox_ros_driver2（这个东东记得编译的时候拿出来不然会报错，单独编译一次即可）
  （去ws_livox下面启动哦，git的时候路径不要在这个下面）
  ...
  ./build.sh humble
   2.1 修改/src/livox_ros_driver2/config/mid360配置文件（ps:我用的是mid360,其他的可能要做适配）
       可以看看参考配置：
         {
      "lidar_summary_info" : {
        "lidar_type": 8
...
5.启动，启动，全部启动！！！
  ...
  （ps：使用rviz2可以看到点云输出了）
6.Autonomous Exploration导航算法(ps:在Autonomous Exploration工作空间下)
  sudo apt update
